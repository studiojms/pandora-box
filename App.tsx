...
import { Idea, ViewState, IdeaType, Language, SortOption, UserProgress, Notification, UserRole } from './types';
...
import { Box, Plus, Search, Layers, Globe, Heart, LogOut, Filter, ArrowUpDown, CheckCircle2, ChevronRight, Trophy, CloudOff, RefreshCw, Bell, User as UserIcon, Zap, LayoutDashboard, Languages, Building2, Rocket } from 'lucide-react';
...
function PandoraBoxApp() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      return () => { unsub.then(f => f && typeof f === 'function' && f()); };
    } else {
      setProgress(null);
      setNotifications([]);
    }
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-100 flex flex-col font-sans overflow-x-hidden">
      <nav className="sticky top-0 z-40 bg-[#0f172a]/95 backdrop-blur-lg border-b border-slate-800">
        <div className="max-w-7xl mx-auto px-4 h-16 md:h-20 flex items-center justify-between">
          <div onClick={() => setView({ type: 'FEED' })} className="flex items-center space-x-2 md:space-x-3 cursor-pointer group">
            ...
            <div className="flex flex-col">
                ...
                <span className="text-[8px] md:text-[10px] text-slate-400 tracking-widest uppercase mt-1 hidden xs:block">The Idea Network</span>
            </div>
          </div>

          <div className="flex items-center space-x-2 md:space-x-4">
            {/* For Teams Link for Guests/Individuals */}
                ...
                {showNotifs && (
                  <div className="absolute right-0 mt-3 w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-fade-in z-50">
                    <div className="p-4 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
                      ...
                      <button onClick={() => backend.markAllNotificationsRead(user.id)} className="text-[10px] font-bold text-purple-400 hover:underline">{t.markAllRead}</button>
                    </div>
                    <div className="max-h-96 overflow-y-auto custom-scrollbar">
                      {notifications.length === 0 && <p className="p-8 text-center text-slate-500 text-xs italic">{t.noNotifications}</p>}
                      {notifications.map(n => (
                        <div key={n.id} onClick={() => { backend.markNotificationRead(n.id); setView({ type: 'IDEA_DETAIL', ideaId: n.ideaId }); setShowNotifs(false); }} className={`p-4 border-b border-slate-800/50 cursor-pointer hover:bg-slate-800/50 transition-colors ${!n.read ? 'bg-purple-900/5' : ''}`}>
                          <p className="text-xs text-slate-300 leading-snug"><span className="font-bold text-white">@{n.fromUserName}</span> {t.notifContributorAdded} <span className="font-bold text-purple-400">"{n.ideaTitle}"</span></p>
            ...
            <button onClick={() => { if(!user) openAuthModal(); else setView({ type: 'FAVORITES' }); }} className={`p-2 rounded-full transition-all ${view.type === 'FAVORITES' ? 'text-rose-500 bg-rose-500/10' : 'text-slate-400 hover:text-white'}`} title={t.favorites}>
              <Heart size={20} fill={view.type === 'FAVORITES' ? 'currentColor' : 'none'} />
            </button>

            <button onClick={handleCreateRequest} className="bg-white text-slate-950 hover:bg-slate-200 p-2 md:px-5 md:py-2.5 rounded-full text-sm font-bold transition-all flex items-center space-x-2">
              <Plus size={18} /><span className="hidden sm:inline">{t.contribute}</span>
            </button>
            ...
            {user ? (
              <div className="flex items-center space-x-2 md:space-x-3 pl-2 border-l border-slate-800">
                <button onClick={() => handleNavigateToUser(user.name)}>
                  <img src={user.avatar} className="w-8 h-8 md:w-9 md:h-9 rounded-full border border-purple-500 hover:scale-110 transition-transform" alt={user.name} />
                </button>
              </div>
            ) : (
              <button onClick={openAuthModal} className="text-slate-200 hover:text-white font-bold text-sm px-3 md:px-4">{t.signIn}</button>
            )}
          </div>
        </div>
      </nav>
      ...
      <main className="flex-grow container mx-auto px-4 pt-6 md:pt-10 pb-20 max-w-7xl">
        {(view.type === 'FEED' || view.type === 'FAVORITES') && (
          <div className="space-y-8 md:space-y-12 animate-fade-in">
            <div className="text-center py-6 md:py-12 space-y-4 md:space-y-6">
              <h1 className="text-4xl md:text-7xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 via-white to-cyan-400 tracking-tight leading-tight">{view.type === 'FAVORITES' ? t.favorites : t.tagline}</h1>
              <p className="text-slate-400 max-w-2xl mx-auto text-base md:text-xl leading-relaxed px-4">{view.type === 'FAVORITES' ? '' : t.subTagline}</p>
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-4 mt-8 md:mt-10">
                <div className="relative w-full max-w-xl group">
                  ...
                  <div className="relative flex items-center bg-slate-900 border border-slate-800 rounded-2xl">
                      ...
                      <input type="text" placeholder={t.searchPlaceholder} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-transparent py-3 md:py-4 pl-11 md:pl-12 pr-4 text-white placeholder-slate-600 focus:outline-none text-base md:text-lg rounded-2xl" />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 p-2 rounded-2xl w-full md:w-auto overflow-hidden">
                   ...
                   <select value={sortBy} onChange={(e) => setSortBy(e.target.value as SortOption)} className="bg-transparent text-xs md:text-sm text-slate-300 font-bold focus:outline-none cursor-pointer hover:text-white pr-4 appearance-none w-full md:w-auto">
            ...
            {/* Teams Marketing Banner inside the Feed */}
                ...
                <div className="relative bg-slate-900 border border-slate-800 rounded-[32px] p-8 md:p-12 overflow-hidden flex flex-col md:flex-row items-center gap-8">
                  ...
                  <div className="flex-1 relative z-10 text-center md:text-left">
                    <div className="flex items-center justify-center md:justify-start gap-2 mb-4">
                       <span className="bg-amber-500/20 text-amber-400 text-[10px] font-bold px-2 py-1 rounded border border-amber-500/30 uppercase tracking-widest">Enterprise Innovation</span>
                    </div>
                    <h2 className="text-2xl md:text-4xl font-bold text-white mb-4 tracking-tight">{t.teamsLandingHero}</h2>
                  ...
                  <div className="relative z-10 flex flex-col items-center gap-4">
                    ...
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Unlock Private Workspaces</span>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              ...
              {filteredIdeas.length === 0 && !loading && (
                 <div className="col-span-full text-center py-16 md:py-24 text-slate-600 border-2 border-dashed border-slate-800 rounded-3xl mx-4">
                    ...
                    <p className="text-lg md:text-xl font-medium">{view.type === 'FAVORITES' ? t.noFavorites : t.noIdeas}</p>
                 </div>
              )}
              {loading && <div className="col-span-full text-center py-12 text-slate-500 animate-pulse">Consulting the Oracle...</div>}
            </div>
          </div>
        )}
        ...
        {view.type === 'PRO_PLAN' && (
          <ProPlanPage onBack={() => setView({ type: 'FEED' })} />
        )}
      ...
      {isCreateModalOpen && <CreateIdeaModal onClose={() => setIsCreateModalOpen(false)} onSubmit={handleCreateSubmit} />}
