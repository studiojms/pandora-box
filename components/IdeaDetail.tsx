...
interface IdeaDetailProps {
  ...
  onRelateRequest?: (parentIdea: Idea) => void;
}
...
export const IdeaDetail: React.FC<IdeaDetailProps> = ({
  ...
  const handleAddContributor = async () => {
    ...
    try {
      ...
      // Fix: Used optional chaining to avoid "unknown" type access errors
      else if (idea.contributorIds?.includes(targetUser.id) || targetUser.id === idea.authorId) {
        setContributorError('User already a member');
      } else {
        await backend.addContributor(idea.id, idea.title, user.name, targetUser.id);
        // Fix: Removed unnecessary and problematic casts
        onUpdateIdea({ ...idea, contributorIds: [...(idea.contributorIds || []), targetUser.id] });
    ...
    <div className="max-w-5xl mx-auto pb-20 animate-fade-in px-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center gap-2">
          {onRelateRequest && (
            <button
              onClick={() => onRelateRequest(idea)}
              className={`flex-1 sm:flex-none flex items-center justify-center space-x-2 px-6 py-2 rounded-full font-bold text-xs transition-all shadow-lg ${isProblem ? 'bg-cyan-600 hover:bg-cyan-500 text-white' : 'bg-purple-600 hover:bg-purple-500 text-white'}`}
            >
              <PlusCircle size={16} />
              <span>{isProblem ? t.proposeSolution : t.relateIdea}</span>
            </button>
          )}
          ...
          <button
            onClick={handleShare}
            className="flex-1 sm:flex-none flex items-center justify-center space-x-2 px-4 py-2 rounded-full border border-slate-700 bg-slate-800 text-slate-400 hover:text-white transition-all"
          >
      ...
      <div
        className={`relative overflow-hidden rounded-3xl bg-slate-800/80 backdrop-blur border-l-4 md:border-l-8 ${borderColor} p-6 md:p-12 mb-8 shadow-2xl`}
      >
        <div className="flex items-center space-x-3 mb-4 md:mb-6">
          <span
            className={`inline-block px-2 py-0.5 md:px-3 md:py-1 rounded text-[9px] md:text-[10px] font-bold uppercase tracking-widest ${isProblem ? 'bg-rose-950 text-rose-400 border border-rose-900' : 'bg-cyan-950 text-cyan-400 border border-cyan-900'}`}
          >
        ...
        <p className="text-base md:text-xl text-slate-300 leading-relaxed mb-6 md:mb-8 max-w-3xl border-l-2 border-slate-600 pl-4 md:pl-6">
          {idea.description}
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 text-sm text-slate-400 w-full">
          ...
          >
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold text-slate-900 shadow-lg group-hover/profile:scale-110 transition-transform ${isProblem ? 'bg-rose-400' : 'bg-cyan-400'}`}
            >
          ...
          {!isDraft && (
            <button
              onClick={handleEcho}
              className="w-full sm:w-auto flex items-center justify-center space-x-2 sm:ml-auto bg-slate-900/50 px-5 py-3 rounded-2xl border border-slate-700 hover:border-slate-500 hover:bg-slate-900 transition-all active:scale-95 shadow-lg group"
            >
              <Activity
                size={20}
                className={`transition-transform group-hover:scale-125 ${isProblem ? 'text-rose-400' : 'text-cyan-400'}`}
              />
      ...
      <div className="flex overflow-x-auto no-scrollbar space-x-6 md:space-x-8 border-b border-slate-800 mb-8 px-2 whitespace-nowrap">
        {[
          { id: 'overview', label: t.connectionsTab },
          ...
          <button
            ...
            className={`pb-4 px-1 text-xs md:text-sm font-bold tracking-wide transition-all relative ${activeTab === tab.id ? 'text-white' : 'text-slate-500 hover:text-slate-300'}`}
          >
            {tab.label}
            {activeTab === tab.id && (
              <div
                className={`absolute bottom-0 left-0 w-full h-1 rounded-t-full ${tab.id === 'business' ? 'bg-purple-500' : isProblem ? 'bg-rose-500' : 'bg-cyan-500'}`}
              />
            )}
          </button>
        ))}
                ...
                <div className="bg-slate-900/50 rounded-2xl p-6 border border-slate-800">
                  ...
                  <div className="space-y-4">
                    {connections.map((conn, idx) => (
                      <div
                        key={idx}
                        onClick={() => onNavigateTo(conn.relatedIdea)}
                        className={`flex items-center gap-4 p-4 rounded-xl border bg-slate-800/40 cursor-pointer transition-all hover:bg-slate-800`}
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span
                              className={`text-[9px] font-bold px-1.5 py-0.5 rounded border uppercase ${conn.relatedIdea.type === IdeaType.PROBLEM ? 'bg-rose-950/30 text-rose-400 border-rose-900' : 'bg-cyan-950/30 text-cyan-400 border-cyan-900'}`}
                            >
                              {conn.relatedIdea.type}
                            </span>
                            <h4 className="text-white font-semibold text-xs md:text-sm line-clamp-1">
                              {conn.relatedIdea.title}
                            </h4>
                          </div>
                          <p className="text-slate-400 text-[10px] md:text-xs line-clamp-1">
                            {conn.relatedIdea.description}
                          </p>
                        </div>
              ...
              <div className="bg-slate-900/50 rounded-2xl p-6 md:p-8 border border-slate-800 relative overflow-hidden">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 relative z-10">
                  ...
                  <button
                    ...
                    className="text-[10px] font-bold bg-slate-800 text-white px-4 py-2 rounded-xl border border-slate-700 hover:bg-slate-700 transition-colors flex items-center justify-center"
                  >
                ...
                <div className="space-y-3 relative z-10">
                  {aiSuggestions.map((s, i) => (
                    <div
                      key={i}
                      className="p-4 bg-slate-800/40 border border-slate-700 rounded-2xl text-slate-300 text-sm leading-relaxed"
                    >
                      {s}
                    </div>
                  ))}
                  {aiSuggestions.length === 0 && !loadingSuggestions && (
                    <p className="text-slate-500 italic text-center py-4 text-sm">{t.emptyBrainstorm}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-900/50 rounded-2xl p-6 border border-slate-800">
                ...
                <div className="space-y-3">
                  ...
                  {/* Fix: Simplified type narrowing for contributorIds to avoid unknown type errors */}
                  {idea.contributorIds &&
                    idea.contributorIds.length > 0 &&
                    idea.contributorIds.map((id, i) => (
          ...
          <div className="space-y-8 max-w-3xl">
            <div className="bg-slate-900/50 p-4 md:p-6 rounded-2xl border border-slate-800">
              <textarea
                ...
                rows={3}
                className="w-full bg-slate-800 border border-slate-700 rounded-xl p-4 text-white focus:outline-none focus:border-purple-500 transition-colors resize-none mb-4 text-sm"
              />
              <div className="flex justify-end">
                <button
                  onClick={handlePostComment}
                  className="w-full sm:w-auto bg-purple-600 hover:bg-purple-500 text-white px-6 py-2.5 rounded-xl font-bold transition-all flex items-center justify-center gap-2"
                >
            ...
            <div className="space-y-4">
              {/* Fix: Safely checking length of comments array to avoid type errors */}
              {comments && comments.length === 0 && (
                <p className="text-center py-10 text-slate-600 text-sm italic">{t.noComments}</p>
              )}
              {comments.map((c) => (
                <div
                  key={c.id}
                  className="bg-slate-800/30 p-4 md:p-5 rounded-2xl border border-slate-700 relative group/comment"
                >
                  <div className="flex justify-between items-center mb-2">
                    <button
                      onClick={() => onUserClick && onUserClick(c.author)}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {contributions.map((ct) => (
                <div
                  key={ct.id}
                  className="bg-slate-800/30 p-5 rounded-2xl border border-slate-700 hover:border-emerald-500/50 transition-all flex flex-col h-full group"
                >
                  ...
                  <div className="mt-auto pt-4 flex items-center justify-between border-t border-slate-700/50">
                    <button
                      onClick={() => onUserClick && onUserClick(ct.author)}
                      className="text-[9px] text-slate-500 hover:text-white transition-colors"
                    >
                      @{ct.author}
                    </button>
                    {ct.content.startsWith('http') && (
                      <a
                        href={ct.content}
                        target="_blank"
                        rel="noreferrer"
                        className="text-emerald-400 hover:text-white transition-all"
                      >
              ...
              {/* Fix: Safely checking length of contributions array to avoid type errors */}
              {contributions && contributions.length === 0 && (
                <p className="col-span-full text-center py-10 text-slate-600 text-sm italic">{t.noContributions}</p>
              )}
            </div>
          </div>
        )}
    ...
    <div className="space-y-3">
      <div className="relative aspect-video rounded-[32px] overflow-hidden bg-slate-900 border border-slate-800 group shadow-2xl">
        {selected.type === 'IMAGE' ? (
          ...
          <video src={selected.url} className="w-full h-full object-contain" controls autoPlay muted loop />
        )}
