...
export const UserProfile: React.FC<UserProfileProps> = ({
  ...
  const prepareInteractionBreakdown = () => {
    return [
      ...
      {
        name: t.discussionTab.split(' ')[0],
        value: analyticsData.filter((e) => e.type === InteractionType.COMMENT).length,
      },
  ...
  if (!profileUser) {
    return (
      <div className="py-24 md:py-32 text-center text-slate-500 flex flex-col items-center px-4">
        ...
        <button onClick={onBack} className="mt-8 text-purple-400 font-bold hover:underline">
          Return to Feed
        </button>
      </div>
    );
    ...
    <div className="max-w-6xl mx-auto pb-20 animate-fade-in px-4">
              ...
              <div className="w-24 h-24 md:w-40 md:h-40 rounded-2xl md:rounded-3xl bg-gradient-to-br from-purple-600 to-cyan-600 p-1 shadow-2xl relative z-10">
                <div className="w-full h-full bg-slate-900 rounded-[14px] md:rounded-[22px] flex items-center justify-center overflow-hidden">
                  {editData.avatar || profileUser.avatar ? (
                    <img
                      src={editData.avatar || profileUser.avatar}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {isEditing ? (
                ...
                <div className="space-y-4">
                  <p className="text-slate-300 text-base md:text-lg leading-relaxed text-center md:text-left">
                    {profileUser.bio ||
                      'This creative mind is still composing their introduction to the Pandora network.'}
                  </p>
                  {profileUser.website && (
            ...
            <div className="flex flex-col gap-4">
              ...
              {isOwnProfile && profileUser.role === UserRole.INDIVIDUAL && (
                <div
                  className="bg-slate-900/40 p-5 rounded-2xl border border-amber-500/20 hover:border-amber-500/40 transition-all cursor-pointer group/org"
      ...
      <div className="flex gap-6 md:gap-8 border-b border-slate-800 mb-8 px-2 overflow-x-auto no-scrollbar whitespace-nowrap">
        ...
        >
          {t.myIdeasTab}
          {activeTab === 'content' && (
            <div className="absolute bottom-0 left-0 w-full h-1 bg-purple-500 rounded-t-full"></div>
          )}
        </button>
        {isOwnProfile && (
          <>
            ...
            >
              ...
              {t.settings}
              {activeTab === 'settings' && (
                <div className="absolute bottom-0 left-0 w-full h-1 bg-emerald-400 rounded-t-full"></div>
              )}
            </button>
          </>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12 animate-fade-in">
          <div className="lg:col-span-2 space-y-6 md:space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              ...
              {ideas.length === 0 && (
                <p className="text-slate-500 italic py-10 text-sm text-center col-span-full">{t.noIdeas}</p>
              )}
            </div>
          </div>
        </div>
          ...
          <div className="bg-slate-900/50 p-8 rounded-3xl border border-slate-800 shadow-xl">
            ...
            <div className="space-y-6">
              <div className="flex items-start justify-between p-4 bg-slate-800/30 rounded-2xl border border-slate-700/50">
                ...
                >
                  <span
                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${userSettings.emailNotificationsEnabled ? 'translate-x-6' : 'translate-x-1'}`}
                  />
                </button>
              </div>
