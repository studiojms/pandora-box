          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span className="bg-amber-500/20 text-amber-400 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest">
                  {company.plan}
                </span>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <StatCard
                label={t.totalPrivateIdeas}
                value={companyIdeas.length}
                icon={<Lock className="text-purple-400" />}
              />
              <StatCard
                label={t.departmentsActive}
              ...
              <div className="bg-slate-900/50 p-8 rounded-3xl border border-slate-800">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {company.departments.map((dept) => (
                    <div
                      key={dept}
                      className="bg-slate-800/40 p-4 rounded-xl border border-slate-700 flex items-center justify-between group hover:border-cyan-500/50 transition-all cursor-pointer"
                    >
                      <span className="text-sm font-bold text-slate-300 group-hover:text-cyan-400 transition-colors">
                        {dept}
                      </span>
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between mb-8">
              ...
              <button
                ...
                className="bg-purple-600 hover:bg-purple-500 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {teams.map((team) => (
                <div
                  key={team.id}
                  className="bg-slate-900/50 p-6 rounded-2xl border border-slate-800 hover:border-purple-500/50 transition-all group"
                >
                  ...
                  <p className="text-slate-500 text-sm mb-6">
                    {team.memberIds.length} {t.membersCount}
                  </p>
                  <div className="flex -space-x-2">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="w-8 h-8 rounded-full bg-slate-800 border-2 border-[#0f172a] flex items-center justify-center text-[10px] font-bold text-slate-500"
                      >
                        M
                      </div>
                    ))}
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              {companyIdeas.length === 0 && (
                <p className="col-span-full py-20 text-center text-slate-600 border-2 border-dashed border-slate-800 rounded-3xl">
                  {t.internalIdeaPoolDesc}
                </p>
              )}
            </div>
          </div>
            ...
            <div className="bg-slate-900/50 rounded-3xl border border-slate-800 overflow-hidden">
              <div className="p-8 border-b border-slate-800 bg-slate-950/20">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-slate-500 font-medium uppercase text-xs">
                    {company.billingCycle === 'MONTHLY' ? t.billingMonthly : t.billingYearly}
                  </span>
                </div>
              </div>
              <div className="p-8 space-y-6">
                ...
                <div className="flex justify-between items-center py-4 border-b border-slate-800/50">
                  ...
                  <span className="text-white font-bold">
                    {t.nextInvoiceDate} - ${company.billingCycle === 'MONTHLY' ? '49.00' : '468.00'}
                  </span>
                </div>
                <div className="pt-4">
                  <button className="text-rose-400 font-bold hover:underline text-sm uppercase tracking-widest">
...
const NavButton: React.FC<{ active: boolean; onClick: () => void; icon: any; label: string }> = ({
  active,
  onClick,
  icon,
  label,
